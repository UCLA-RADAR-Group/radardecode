...
typedef struct {
	...
	int		bytesInpBlk;	/* after unpacking*/
        int		bytesOutReq;	/* for output after transform */
        float   xoff; /* apply frequency offset xoff [Hz, assuming 1 s per cmplx sample] during FFT */
	} RDRFFTI;
 ...
 *
 *       rdrfft  -l lenfft -r numrangebins -p 1,2,4
 *
 *      -l  len fft
 *      -r  number of range bins
 *      -p  packed 1,2,or 4 byte integer data
 *  history:
 ...
 *  jlm added swap_freq()
 *  jlm added capability to apply frequency offset w apply_linear_phase()
*/
        int     xformdir;                       /* +/-1*/
		int		blk;
        int     bytes_inp;
...
*/
        ...
        xformdir = 1; 		/* this is a forward transform*/
...
info.lenfft,info.numbins,info.bytesInpBlk,info.bytesOutReq);
	    fflush(stderr);
			...
			*/
			for (i=0,pc=info.fftBlk;i<info.numbins;i++,pc+=info.lenfft*2)
			  {
                            if (info.xoff != 0) apply_linear_phase(pc, info.xoff, 0.0, 1.0, info.lenfft);		  
			    fftwAo(pc,NULL,info.lenfft,xformdir,1);
			    /* 
			       four1_(pc,&info.lenfft,&xformdir);
			    */
			    /* swap frequencies */
	...
	*/
done: bytes_out = write(STDOUT,(char *)info.fftBlk,info.bytesOutReq);
      if (info.bytesOutReq != bytes_out){
               fprintf(stderr,"radarfft: output error. bytes out %d\n",
			   bytes_out);
               exit(-1);
      }
      fprintf(stderr,"1 block of %d lenfft x %d bins xformed and added\n",
				info.lenfft,info.numbins);
             exit(0);
        /*NOTREACHED*/
}       
...
*/
        ...
        char  *myoptions = "l:r:p:x:";   /* options to search for. :--> needs
                                            an argument*/
        char *USAGE =
   "Usage: radarcplxfft  -l lenffts -r rangeBins -p packed(1-4) [-x freq offset (Hz based on 1 Hz)] ";
 ...
 * input  lenrange*lenfft*size(float)*2(since complex)
*/
		pI->bytesInpBlk=sizeof(float)*2*pI->lenfft*pI->numbins;
		pI->bytesOutReq=pI->bytesInpBlk;
